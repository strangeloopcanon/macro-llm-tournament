...
class ForecastTournamentTests(unittest.TestCase):
    ...
    def test_codex_forecast_provider_retries_timeouts(self):
        cards = build_forecast_cards(
            spf_fixture(),
            variables=["CPI"],
            horizons=[1],
        ...
        with TemporaryDirectory() as tmp, patch.dict(
            "os.environ",
            {"CODEX_CLI_BIN": "/tmp/codex", "LLM_JSON_ATTEMPTS": "2"},
        ):
            client = ForecastLLMClient("codex_cli", "gpt-5.5", Path(tmp), mode="live", max_live_calls=2)
