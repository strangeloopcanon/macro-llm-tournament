...
The answer is now narrower, cleaner, and more useful. Frontier LLMs are good macro belief engines. They forecast hidden macro outcomes better than strong statistical baselines, survive direct recall audits, and update beliefs in the right direction when given a person's prior beliefs. They are not reliable generators of individual household heterogeneity from demographics or backstories. The best behavior interfaces are measured or scheduled functions that deterministic code executes against household state. The real-data bridge gets the Phase 4 economy close to adaptive expectations, and horizon-aligned retrospective replays can beat it.

The recursive economy now carries persistent real SCE households through five post-cutoff monthly origins, feeds simulated state forward, preserves accounting, and scores 10 first-release macro targets per scored origin. The latest experiment corrected the population weights and compared the same economy with 81 and 200 households. The corrected 81 path has the best absolute LLM score, `0.546550`, and narrowly beats its adaptive twin at `0.548858`. The 200 path scores `0.548996` against adaptive `0.548379`. Scaling up therefore makes the LLM score `0.45%` worse, not better. But that difference falls inside the pre-registered 1% near-tie band, so 200 households are promoted for population coverage: effective sample size rises from `46.53` to `123.58`, while the largest household weight falls from `6.41%` to `2.15%`. Both LLM-versus-adaptive uncertainty intervals cross zero. The promoted 200-household economy is frozen for a one-shot June 2026 test once the complete first-release target set exists.

The short version:

**LLMs are belief updaters and policy-function authors, not reliable household simulacra. Real data supplies the heterogeneity; the model updates beliefs; code turns them into feasible actions and macro feedback. That economy now runs with 200 persistent households and nearly matches a strong adaptive benchmark. More households improve representation, not predictive accuracy. The fresh frozen-vintage verdict is still open.**
...
| Does the recursive household economy work end to end? | Yes. | Both corrected cohorts complete five monthly origins from real SCE states through GPT-5.5 belief updates, empirical spending behavior, deterministic accounting, macro feedback, and 10-target first-release scoring. The 15 accepted calls have zero retries; all accounting residuals remain at numerical tolerance. |
| Does increasing the ecology from 81 to 200 improve macro accuracy? | No. | The corrected 81 LLM score is `0.546550`; 200 scores `0.548996`, `0.45%` worse. Scale sharply improves representation, raising effective sample size from `46.53` to `123.58` and cutting the largest normalized weight from `6.41%` to `2.15%`. The locked near-tie rule therefore promotes 200. |
| Does the promoted recursive LLM economy beat adaptive expectations? | Not yet. | The promoted 200 path scores `0.548996` versus adaptive `0.548379`, a `0.11%` loss. Its origin-block interval is `[-0.00655, 0.00757]`. The corrected 81 path leads adaptive by `0.42%`, but its interval also crosses zero. |
| Is the economy ready as a validated macro simulator? | No. | It is a real recursive simulation with a pre-registered developmental promotion. The exact 200-household path is frozen for one June 2026 confirmation, which remains unspent until the complete first-release bundle is available. |
...
### Recursive macro economy: corrected weights and household scale

The earlier Phase 4 surfaces replayed a short belief panel through a scoring bridge. The current lane is a genuine recursive simulation. It begins with real SCE household priors, demographics, and survey weights, plus coarse balance-sheet states mapped from survey groups. At each monthly origin, GPT-5.5 updates household inflation, real-income, unemployment-risk, and confidence beliefs from origin-visible information. Empirical bridge v4 converts those changes into consumption pressure. Deterministic code executes consumption, saving, debt repayment, liquidity, income, employment, inflation, and policy-rate transitions, checks accounting, and carries state into the next origin. The adaptive twin changes only the belief updater.

The frozen January-May 2026 bundle uses common-month first-release targets. January is warmup; February-May are scored against 10 targets in five equally weighted families: demand, balance sheets, labor, prices, and income/policy. Every one of the 50 catalogue rows was first released after GPT-5.5's December 1, 2025 cutoff. Prompts contain origin-visible history and simulated state, never target realizations, target aliases, survey weights, or selection weights.

The first recursive winner used 81 households, but its aggregation carried raw SCE weights without correcting for disproportionate stratified subsampling. That result, `0.549789` LLM versus `0.548667` adaptive, is now historical context only. The corrected experiment begins from the 886 respondents who had an April 2025 observation and an immediate March prior available at the January 2026 origin. A locked seed produces a nested 81-household core and 200-household master sample across income, age, and education strata. Each aggregation weight is the SCE survey weight multiplied by the eligible-to-sampled count in its stratum, then normalized. The LLM never sees those weights.

Both corrected cohorts run the same mechanism: empirical bridge v4; global belief gain `3.0`; inflation gain `1.5`; income gain `0.5`; unemployment-risk gain `1.0`; full origin-visible policy-state assimilation; and policy-rate smoothing `0.85`. The 81 path uses one model batch per origin. The 200 path uses two fixed batches of 100, with exact identity coverage required before the economy advances.

| Cohort | LLM MacroScore | Adaptive | LLM minus adaptive | Direction, LLM | Effective sample size | Largest weight |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Historical uncorrected 81 | `0.549789` | `0.548667` | `+0.001122` | `49.17%` | `55.34` | `4.72%` |
| Corrected 81 | **`0.546550`** | `0.548858` | `-0.002308` | `45.00%` | `46.53` | `6.41%` |
| Corrected 200 | `0.548996` | `0.548379` | `+0.000617` | **`50.00%`** | **`123.58`** | **`2.15%`** |

Lower is better. The corrected 81 economy has the best absolute LLM score and beats its adaptive twin by `0.42%`. Its advantage comes mainly from the demand family; its origin-block interval `[-0.00794, 0.00178]` still crosses zero. The 200 economy is `0.45%` worse than corrected 81 and trails its adaptive twin by `0.11%`, with interval `[-0.00655, 0.00757]`. Scaling up helps balance-sheet fit and direction accuracy, but gives back more on demand and income/policy. Household count was not the main accuracy bottleneck.

The pre-registered rule minimizes the absolute LLM score, but treats a relative difference of at most 1% as a near tie and then prefers 200 for population coverage. The observed gap is `0.4477%`. The promotion receipt therefore selects `corrected200`: not because it predicts better, but because the score difference is small and its weighted representation is much less concentrated.

The live comparison spent exactly 15 accepted GPT-5.5 calls through Codex CLI at high reasoning effort: five for corrected 81 and ten for corrected 200. There were no failed attempts, semantic retries, replayed records, or ordinary cache hits. The independent comparison validator reconstructs forecasts from period paths, binds targets to the frozen bundle, recomputes score rows, verifies raw/cache/ledger hashes and prompt contents, and rebuilds household and aggregate accounting. Maximum recomputed residuals are `2.09e-11` for 81 and `4.73e-11` for 200.

Mechanism search remains closed. The promoted 200-household cohort, provider, model, adjusted weights, two-batch layout, prompts, empirical bridge, ten replay records, target contract, executable source tree, and hashes are frozen in `configs/dynamic_macro/confirmatory_june_2026_v1.json`. The one-shot runner will replay five periods and permit exactly two accepted June batches, with two schema retries per batch. It currently fails before creating a receipt because the June 10-target first-release bundle is incomplete; the final required [BEA Personal Income and Outlays release is scheduled for July 30, 2026](https://www.bea.gov/news/schedule/).
...
10. **A recursive 200-household macroeconomy now runs over post-cutoff monthly vintages.** It carries persistent beliefs, household balance sheets, demand, labor, prices, income, and policy state through five origins; exact two-batch coverage and population-weighted accounting pass. Relative to corrected 81, effective sample size rises from `46.53` to `123.58` and the largest normalized household weight falls from `6.41%` to `2.15%`.
...
12. **Increasing the ecology from 81 to 200 does not improve January-May macro accuracy.** Corrected 81 has the better LLM score (`0.546550` versus `0.548996`). The promoted 200 path remains `0.11%` behind adaptive, and its uncertainty interval crosses zero. It is promoted by the locked near-tie representation rule, not by an accuracy win.

The full claim remains open:

**We have built a persistent, population-weighted, recursive LLM economy, but have not yet shown confirmatorily that it predicts macro behavior better than a strong adaptive benchmark.** Corrected 81 leads adaptive on the point estimate; the promoted 200 path trails it slightly; neither interval separates the twins. The scale experiment says the remaining problem is not too few households. The next evidence is the locked June run.

## Recommended Next Work

The next phase is already specified.

1. Do not tune another mechanism or household count on January-May. The promoted 200-household path is frozen.
2. Build the complete January-June common-month bundle only after all 10 June first releases exist, expected July 30, 2026.
3. Run `make dynamic-macro-confirmatory-june` once. Five banked periods replay in 10 batches; GPT-5.5 supplies exactly two accepted June batches; only June's 10 targets are scored.
...
The forecast evidence is already strong. Persona generation, point-behavior elicitation, and household-scale search are closed. The current question is now clean: does the locked 200-household recursive economy carry its developmental near-tie into one fresh month?
...
`persistent_households.py` builds the nested corrected 81/200 cohorts from the 886 real-time-eligible SCE respondents. It separates private respondent matching from stable public IDs, stores append-only observed history, preserves nonrespondents as living households, and recomputes dynamic groups from current state. `frozen_vintage_bundle.py` freezes six canonical CSV payloads and validates their full target catalogue, source requests, first-release vintages, contamination labels, and hashes. `dynamic_macro_economy.py` runs the matched twins and emits household, final-state, prompt, belief, decision, period, accounting, forecast, error, target, family, origin, replay, live-attempt, and manifest artifacts. Batches contain at most 100 households and must partition the cohort exactly once per period.

The corrected live runs are `outputs/dynamic_macro_household_scale_81_gpt55_live_v1/` and `outputs/dynamic_macro_household_scale_200_gpt55_live_v1/`; their pre-registration is `configs/dynamic_macro/household_scale_development_v1.json`. `dynamic_macro_household_scale.py` and its dedicated validation layer reconstruct the score surface and accounting from primitive artifacts before writing `outputs/dynamic_macro_household_scale_comparison_v1/` and its promotion receipt. The tracked compact evidence is `reports/dynamic_macro_household_scale_v1_evidence.json`. The promoted records are banked under `work/dynamic_macro/banked_household_scale_v1/`; `make dynamic-macro-incumbent-replay` reproduces the 200-household path with zero provider calls.

The June lock is `configs/dynamic_macro/confirmatory_june_2026_v1.json`, with the exact Python source tree frozen in `reports/dynamic_macro_confirmation_source_lock_v1.json`. `dynamic_macro_confirmatory.py` validates every locked development input, the executable source, the complete canonical January-June bundle, both June prompt batches, and every score artifact before atomically completing the one-shot receipt. The historical uncorrected 81 evidence remains in `reports/dynamic_macro_development_v1_evidence.json` and cannot be promoted.

### Reproducibility notes

The canonical report in the repository is `reports/macro_simulation_report.md`. The live elicitation campaign artifacts are under `outputs/persona_elicitation_campaign/`. The Phase 4 fixture artifacts are under `outputs/phase4_matched_twins_fixture/`. The Phase 4 v4 diagnostic artifacts are under `outputs/phase4_v4_diagnostics/`. The research-path retrospective is `reports/research_retrospective.md`. The full local contract passes `315` tests. The data-asset event stream (downloads, derivations, run manifests, all timestamped) is `data_provenance/data_events.jsonl`.
