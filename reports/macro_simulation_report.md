...
The project has now shifted from one-gate validation to full-economy tournament search. The runner is `python3 -m macro_llm_tournament.macro_tournament`, with the tracked development spec at `configs/macro_tournament/development_v1.json`. It searches only retrospective, already-spent Phase 4 surfaces; the next newly scoreable FRED month remains reserved for a future confirmatory run. Adaptive/persistence-style rows are retained as diagnostics, but they do not veto LLM candidate selection. The hard vetoes are accounting failure, malformed provenance, mapping drift, or leakage.
