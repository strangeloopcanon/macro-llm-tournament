...
The agent-behavior claim is still not done. The behavior economy passes accounting and mechanism checks, and the live synthetic-SCE persona panel now clears demographic-gradient and spread checks. But a 3-call live behavior gate shows raw GPT-5.5 household actions do not beat the liquidity rule on the current target catalog. The useful result is narrower: LLM-shaped actions plus explicit liquidity calibration improve the cell-level MPC shape, while the raw behavior policy is not yet good enough.
...
- Behavior economy: HANK-lite demand economy with deterministic accounting and bounded belief inputs, plus a 3-call live GPT-5.5 behavior gate scored against rule baselines.
...
The first live GPT-5.5 behavior gate is not a win for raw LLM behavior. Raw GPT-5.5 loses to the liquidity rule on aggregate targets: range RMSE `0.0762` versus `0.0560`, a delta of `0.0203`. It also loses on cell-level MPC targets: range RMSE `0.0387` versus `0.0272`, a delta of `0.0115`.

The interesting result is in the ablations. A residual-over-liquidity blend ties the liquidity rule on aggregate targets, and a 50 percent liquidity-prior blend beats it on cell-level MPC targets: range RMSE `0.0073` versus `0.0272`. That says the current live LLM actions contain some usable cross-sectional shape, but only after explicit liquidity calibration.

It does not yet mean the simulated economy predicts real macro behavior better than a strong empirical model. The missing bridge is still behavior validation: calibrated behavior runs must beat the best rule baseline on MPCs, saving, debt repayment, liquidity shifts, and response dynamics without needing a post-hoc blend that merely restores the rule.

## Current Claim

The sendable claim is:

> On a 147-card held-out, date-free vintage macro split, raw GPT-5.5 and GPT-5.4 belief forecasts beat no-change, rolling mean, rolling trend, AR(2), and recursive least-squares baselines in aggregate. The win survives origin-cluster bootstrapping. Live recall probes show zero exact realized-value recall and only modest qualitative path recall. The behavior economy is accounting-safe and mechanism-complete, but raw GPT-5.5 behavior does not yet beat the liquidity-rule baseline. The persona/behavior layer still needs distribution and liquidity calibration before we can claim broad macro-agent predictive validity.
...
3. Feed calibrated belief distributions into the behavior economy and require raw LLM behavior and any pre-registered calibrated variant to be scored separately against the behavior baseline scoreboard.
