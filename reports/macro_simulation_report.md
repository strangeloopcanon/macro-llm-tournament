...
| Does full-economy tournament search improve the incumbent? | Yes, modestly and retrospectively. | The new `macro_tournament` runner scored `5,670` candidates across two spent Phase 4 surfaces with zero live calls and no disqualifications. The winner keeps the v5 empirical bridge, raises belief-update gain on inflation/unemployment, lowers income gain, and uses stronger closed-loop feedback. Mean LLM scaled RMSE improves from incumbent `0.7249` to `0.7208`. This winner is now promoted as the replayable `macro_incumbent_v1` spec. |
| Does extended tournament search find a real optimum? | Partially, with a clear overfitting boundary. | The v2 grid finds an interior optimum (v4 bridge, global gain `3.0`, mean `0.7087`, beating the adaptive diagnostic mean `0.7327`). But a frontier probe shows candidates past that region score "better" only by surface-fitting: cross-surface rank correlation flips from `+0.955` to `-0.75`. The moderate amplification region is stable across both December panel draws; the exact per-target knobs are not. |
| Does the locked fresh FRED confirmatory month reproduce the retrospective Phase 4 win? | No. | The one-shot confirmatory run scored only the `2026-02-15` as-of card, carrying earlier cards only for state alignment. The unit-gain v5 comparator is the best LLM candidate (`2.1202` scaled RMSE), narrowly ahead of the conservative v4 amplified candidate (`2.1256`), but adaptive is better for both (`2.1019`-`2.1009`). Accounting passes and the spent-surface registry blocks reruns. |
| Is the economy ready as a validated macro simulator? | Not yet. | The accounting-safe demand sandbox works, and Phase 4 has retrospective matched-twin wins after horizon alignment. The first fresh confirmatory FRED month failed, so the validated claim is still open. |
...
The project has now shifted from one-gate validation to full-economy tournament search. The runner is `python3 -m macro_llm_tournament.macro_tournament`, with tracked development specs under `configs/macro_tournament/`. Development specs search only retrospective, already-spent Phase 4 surfaces. The first locked fresh FRED confirmatory month has now been spent separately by `configs/macro_tournament/confirmatory_fred_2026_02_v1.json` and failed. Adaptive/persistence-style rows are retained as diagnostics, but they do not veto LLM candidate selection. The hard vetoes are accounting failure, malformed provenance, mapping drift, or leakage.
...
The winning candidate is `cand_1623eb882b6a`. It keeps the empirical bridge v5 behavior mechanism, uses closed-loop feedback with feedback gain multiplier `1.5`, and applies deterministic belief-update gains: global `1.50`, inflation `1.25`, unemployment-risk `1.25`, income `0.75`. Its mean LLM scaled RMSE across the two retrospective surfaces is `0.7208`, versus `0.7249` for the incumbent v5 bridge with unit gains. The improvement is small (`0.0042` scaled RMSE), but directionally useful because it comes from full-economy scoring rather than a hand-picked component gate. This candidate is now promoted into `configs/macro_tournament/incumbent_v1.json` and can be replayed with `make macro-incumbent-v1`.

Surface detail matters. On the original one-card replay, the winner is slightly worse than the incumbent (`0.5984` versus `0.5970`) and still worse than adaptive (`0.5688`). On the aligned two-card replay, it improves the LLM path from `0.8529` to `0.8431` and beats adaptive (`0.8976`). So the tournament did not discover a new behavior architecture; it found that the best current economy is still the stabilized empirical bridge, with stronger belief movement and feedback when the horizon is aligned.

### Tournament v2: an interior optimum, and where the search starts lying

The v1 winner sat on the edge of its gain grid (global `1.5` was the maximum searched), so `configs/macro_tournament/development_v2.json` extends global gain to `3.0` and widens the per-target gains (`2,500` candidates, zero live calls, no disqualifications). The v2 winner is `cand_a44fcf25ae3c`: the **unshrunk v4 bridge** with global gain `3.0`, inflation `1.5`, income `0.5`, unemployment `1.5`, feedback `1.5`. Mean LLM scaled RMSE `0.7087` versus incumbent `0.7249`, beating the adaptive diagnostic mean (`0.7327`). Along the winning slice the global-gain frontier is now interior: it peaks at `3.0` and worsens beyond `4.0`. The mechanism flip from v5 back to v4 is informative: the gains partially substitute for v5's ridge shrinkage, so with gains free, the unshrunk bridge plus larger belief movement wins.

A follow-up frontier probe (`configs/macro_tournament/development_v2b_gain_frontier.json`) shows where honest search ends. Pushing further out finds candidates with nominally better means — global `8.0`, inflation `2.0`, income updates fully zeroed scores `0.6621` — but the cross-surface Spearman rank correlation flips from `+0.955` on the broad v1 grid to `-0.75` near the frontier: knobs that help one surface now hurt the other. With roughly 14 scored observations and thousands of candidates, everything past the moderate region is surface-fitting, and those extreme candidates are recorded as a diagnostic, not promoted.

A third run (`configs/macro_tournament/development_v3.json`, same grid as v2) re-scored the search after the December leg was re-elicited under the monthly-vintage fix, using the gated v2 combined panel for the aligned surface. The broad story holds — v4 bridge, global gain `3.0`, inflation `1.5`, income `0.5` — but the preferred unemployment gain flips from `1.5` to `0.5` and the improvement over unit gains shrinks (`0.7240` versus `0.7308`). So the robust finding across both December draws is the moderate amplification region (effective inflation gain around `3`x to `4.5`x, damped income updates), consistent with the compression result from the behavior work: LLM belief updates are directionally right but too timid, and a deterministic gain partially corrects that. The exact per-target knobs are not stable across draws and should not be treated as tuned constants; the pre-registered confirmatory month is the only surface that can settle the candidate choice.
...
8. **Full-economy tournament search can improve the incumbent.** The first macro tournament searches complete LLM economy candidates directly and finds a modest retrospective improvement over the incumbent, without accounting failures or fresh-score leakage. Extended search finds a stable moderate belief-amplification region (v4 bridge, global gain near `3`), consistent with LLM update timidity.
...
11. **The first locked fresh Phase 4 confirmatory month fails.** On the `2026-02-15` as-of FRED surface, both locked LLM-economy candidates lose to adaptive. The conservative amplified candidate does not beat the unit-gain v5 comparator.

The full claim remains open:

**We have not yet shown confirmatorily that an LLM-based simulated economy predicts real macro behavior better than strong empirical alternatives.** We have shown a retrospective version of that result when horizons are aligned, then failed the first locked fresh FRED confirmation. The system is a simulatable, accounting-safe macro sandbox with useful belief and behavior modules; it is not yet a validated macro predictor.
...
3. Treat the `2026-02-15` confirmatory FRED surface as spent. It failed, and the registry now blocks reruns. Do not tune on it.
4. Keep the aligned two-card result in the report as retrospective evidence only. The next confirmatory attempt should wait for a new aligned belief-update panel that clears its own gate or a new pre-registered mechanism change.
5. Keep the Phase 4 v2 output mapping locked unless there is a pre-registered replacement.
