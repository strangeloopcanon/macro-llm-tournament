...
Run the deterministic 12-household fixture:

```bash
make ecology-fixture
```

Reproduce the frozen 200-household path from the accepted response cache with no
new model calls:
...
`ECOLOGY_CACHE`, and `CURRENT_RUN_DIR`, build the origin snapshot, then run
`make ecology-live-200`. Live household calls go only through Codex CLI.
